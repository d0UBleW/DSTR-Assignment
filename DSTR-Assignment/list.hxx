...
/*     /1* BinaryTree<T>* bt = LLToBT<T>(this, (*CompareFn), order); *1/ */
/*     BinaryTree<T>* bt = this->LLToBT<U>((*CompareFn), order); */
/*     /1* List<T>* sortedLL = BTToLL<T>(bt); *1/ */
...
template <typename T>
List<T>* List<T>::Sort(char order, std::string data) {
    /* BinaryTree<T>* bt = LLToBT<T>(this, (*CompareFn), order); */
    BinaryTree<T>* bt = this->LLToBT(order, data);
...
template <typename T>
BinaryTree<T>* List<T>::LLToBT(char order, std::string data) {
    ...
    while (ptr != NULL) {
        ...
        while (temp != NULL) {
            prev = temp;
            result = CompareNumeric(ptr->x, temp->x);
            if (order == 'a') {
                if (result > 0) {
                    temp = temp->next;
                }
