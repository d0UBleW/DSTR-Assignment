			...
			if (!isAdmin)
			{
				if (choice == 1)
				{
					while (true)
					{
						...
						float tutor_rate = tutor->tutor->rating;
						tutor_rate *= count_rate;
						tutor_rate += rate;
						count_rate += 1;
						tutor_rate /= count_rate;
						tutor->tutor->rating = tutor_rate;
				...
				while (!valid)
				{
					...
					if (isChoiceInMenuRange(option, 3))
					{
						...
						else if (option == 2)
						{
							...
							std::getline(std::cin, tutor->tutor->address);
							valid = true;
						}
						else if (option == 3)
						{
							...
							while (true)
							{
								...
								if (isDateValid(date))
								{
									tutor->tutor->terminateDate = date;
									valid = true;
									break;
								}
								else
